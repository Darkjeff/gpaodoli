...
if ($action == 'createmo' && GETPOST('valid')) {
	$fail=0;
	...
	if ($linecount > 0) {
		...
		for ($i = 0; $i < $linecount; $i++) {
			if (GETPOST('choose' . $i, 'alpha') === 'on' && GETPOST('fk_bom_' . $i, 'int') > 0) {
				$bomid=GETPOST('fk_bom_' . $i, 'int');
				$qty = GETPOST('toproduce'.$i, 'int');


				$bom = new BOM($db);
				$resFBom = $bom->fetch($bomid);
				if ($resFBom<0) {
					setEventMessages($bom->error,$bom->errors,'errors');
					$fail++;
				}

				$prod = new Product($db);
				$resfProd = $prod->fetch($bom->fk_product);
				if ($resfProd<0) {
				...
				$mo = new Mo($db);
				$mo->fk_bom = GETPOST('fk_bom_' . $i, 'int');
				$mo->fk_product = $bom->fk_product;
				$mo->qty=$qty;
				$mo->fk_warehouse=$prod->fk_default_warehouse;
				$mo->mrptype=0;
				$resultCreate = $mo->create($user);
				if ($resultCreate<0) {
					setEventMessages($mo->error,$mo->errors,'errors');
					$fail++;
				} else {
					$moCreatedMessages[]=$langs->trans('MOCreated').$mo->getNomUrl();
				}
			}
		}
		if (!$fail) {
			...
			setEventMessages(null, $moCreatedMessages);
			header('Location: replenish_mrp.php');
			exit;
		} else {
			$db->rollback();
		}
...
$sql .= ' AND p.fk_default_warehouse IS NOT NULL';
...
print '<input type="hidden" name="action" value="createmo">';
print '<input type="hidden" name="mode" value="'.$mode.'">';
...
//}
print_liste_field_titre('QtyProductionOngoing', $_SERVER["PHP_SELF"], '', $param, '', '', $sortfield, $sortorder, 'right ');
...
//		}
		...
		// To order
		print '<td class="right"><input type="text" size="4" name="toproduce' . $i . '" value="' . ((!empty($conf->global->STOCK_ALLOW_ADD_LIMIT_STOCK_BY_WAREHOUSE) && $fk_entrepot > 0) > 0 ? $stocktobuywarehouse : $stocktobuy) . '"></td>';

		// BOM
		print '<td class="right">';
		$bom = new Mo($db);
		$morecssBomfield = $bom->fields['fk_bom']['css'];
		$morecssBomfield .= " --success widthcentpercentminusx";
		print $form->selectForForms('Bom:bom/class/bom.class.php:0:(t.status:=:1) AND (t.fk_product:=:' . $prod->id . ')', 'fk_bom_'.$i, $value, $showempty, '', '', $morecss, $moreparam);
		print '</td>';
